...
// Change Quantity Input into Select
function woocommerce_quantity_input($args = [], $product = null, $echo = true)
{
  ...
  $options = '<option>Quantity</option>';

  for (
    $count = $args['min_value'];
    $count <= $args['max_value'];
    $count = $count + $args['step']
  ) {
    // Cart item quantity defined?
    if (
      '' !== $args['input_value'] &&
      $args['input_value'] >= 1 &&
      $count == $args['input_value']
    ) {
      $selected = 'selected';
    } else {
      $selected = '';
  ...
  $string =
    '<div class="quantity"><select class="form-control" name="' .
    $args['input_name'] .
    '">' .
    $options .
    '</select></div>';
